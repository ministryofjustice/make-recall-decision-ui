...
context('Indeterminate Sentence - Edit Date of Sentence Page', () => {
  ...
  describe('Error message display', () => {
      ...
      testForErrorSummary([
        {
          ...
          errorComponentId: 'date-of-sentence-error',
        },
      ])
    ...
    describe('WHEN an invalid date is provided', () => {
      ...
      it('- Blank Date: Error links to day', () => testInvalidDate([], 'day', 'Enter the date of sentence'))
      it('- Blank Day: Error links to day', () =>
        testInvalidDate([undefined, 1, 2000], 'day', 'The date of sentence must include a day'))
      it('- Blank Month: Error links to month', () =>
        testInvalidDate([1, undefined, 2000], 'month', 'The date of sentence must include a month'))
      it('- Blank Year: Error links to year', () =>
        testInvalidDate([1, 1, undefined], 'year', 'The date of sentence must include a year'))
      it('- Blank Day/Month: Error links to day', () =>
        testInvalidDate([undefined, undefined, 2020], 'day', 'The date of sentence must include a day and month'))
      it('- Blank Day/Year: Error links to day', () =>
        testInvalidDate([undefined, 1, undefined], 'day', 'The date of sentence must include a day and year'))
      it('- Blank Month/Year: Error links to month', () =>
        testInvalidDate([1, undefined, undefined], 'month', 'The date of sentence must include a month and year'))
      it('- Year less than the minimum: Error links to year', () =>
        testInvalidDate(
          [1, 1, MIN_VALUE_YEAR - 1],
          'year',
          `The date of sentence must include a year after ${MIN_VALUE_YEAR}`,
        ))
      it('- Day that does not exist: Error links to day', () =>
        testInvalidDate([99, 1, 2020], 'day', 'The date of sentence must have a real day'))
      it('- Month that does not exist: Error links to month', () =>
        testInvalidDate([1, 99, 2020], 'month', 'The date of sentence must have a real month'))
      it('- Year that does not exist: Error links to year', () =>
        testInvalidDate([1, 1, 12345], 'year', 'The date of sentence must have a real year'))
      const now = new Date(Date.now())
      it('- Date in the future: Error links to day', () =>
        testInvalidDate(
          [now.getDate(), now.getMonth() + 1, now.getFullYear() + 1],
          'day',
          'The date of sentence must be today or in the past',
        ))
    })
  })
})
