...
context('Feature flags', () => {
  ...
  it('override a flag', () => {
    ...
    cy.getRadioOptionByLabel(
      "Don't send events to Google Analytics or App Insights for my activity (in prod only)",
      'On'
    ).should('not.be.checked')
    cy.selectRadio("Don't send events to Google Analytics or App Insights for my activity (in prod only)", 'On')
    cy.clickButton('Save')
