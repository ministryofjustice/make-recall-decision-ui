...
import editDateOfSentenceController from './editDateOfSentenceController'
...
const expected = {
  ...
  render: {
    path: 'pages/recommendations/ppcs/indeterminateSentence/edit/editDateOfSentence',
  },
}

describe('Indeterminate Sentence - Edit Date Of Sentence Controller', () => {
  const sentenceId = faker.number.int().toString()
  const originalDateOfSentence = faker.date.past()
  const recommendation = RecommendationResponseGenerator.generate({
    ...
    ppudOffender: {
      sentences: [
        {
          id: sentenceId,
          dateOfSentence: originalDateOfSentence,
        },
      ],
    },
  })
  const expectedDateOfSentence = new Date(recommendation.bookRecallToPpud.ppudIndeterminateSentenceData.dateOfSentence)
  ...
  describe('get', () => {
    ...
    const next = mockNext()
    beforeEach(async () => {
      await editDateOfSentenceController.get(req, res, next)
    })

    describe('Sets response locals correctly:', () => {
      describe('Page Data', () => {
        it('- is defined', async () => expect(res.locals.pageData).toBeDefined())
        it('- Existing date of sentence', async () =>
          expect(res.locals.pageData.existingDateOfSentence).toEqual(originalDateOfSentence.toISOString()))
        it('- Day value to edit', async () => expect(res.locals.pageData.day).toEqual(expectedDateOfSentence.getDate()))
        it('- Month value to edit', async () =>
          expect(res.locals.pageData.month).toEqual(expectedDateOfSentence.getMonth() + 1))
        it('- Year value to edit', async () =>
          expect(res.locals.pageData.year).toEqual(expectedDateOfSentence.getFullYear()))
      })
    })
  ...
  describe('post', () => {
    ...
    describe('Given a valid date is submitted', () => {
      beforeEach(async () => {
        await editDateOfSentenceController.post(validReq, res, next)
      })
          ...
          valuesToSave: {
            bookRecallToPpud: {
              ...
              ppudIndeterminateSentenceData: {
                ...
                dateOfSentence: `${expectedDateParts.year}-${expectedDateParts.month}-${expectedDateParts.day}`,
              },
            },
          },
    ...
    describe('Given an invalid date is submitted', () => {
      ...
      invalidDateTestCases.forEach(({ name, testBody, id, parts }) => {
        ...
        beforeEach(async () => {
          await editDateOfSentenceController.post(invalidReq, res, next)
        })
        describe(`- ${name}`, () => {
          it('Then there is an error defined', async () => expect(invalidReq.session.errors).toBeDefined())
          describe('Error details', () => {
            const dateOfSentenceName = 'dateOfSentence'
            it(`- Contains an error for ${dateOfSentenceName}`, async () =>
              expect(invalidReq.session.errors?.some(e => e.name === dateOfSentenceName)).toBeTruthy())
            const getError = () => invalidReq.session.errors.find(e => e.name === dateOfSentenceName)
            it('- Is supplied with an error message', async () => expect(getError().text).toBeDefined())
            it(`- Has the expected invalid part id: ${dateOfSentenceName}-${id}`, async () =>
              expect(getError().href).toEqual(`#${dateOfSentenceName}-${id}`))
