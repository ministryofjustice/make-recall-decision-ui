...
describe('renderFormOptions', () => {
  ...
  it('FTR56: renders name into recall criteria options', () => {
    const rendered = renderFormOptions({ fullName: 'Barbara' })
    expect(rendered.indeterminateOrExtendedSentenceDetailsFtr56).toEqual([
      {
        detailsLabel: 'Give details:',
    ...
    // leave the original copy unaltered
    expect(formOptions.indeterminateOrExtendedSentenceDetailsFtr56).toEqual([
      {
        detailsLabel: 'Give details:',
