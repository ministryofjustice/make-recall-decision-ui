...
describe('readFeatureFlags', () => {
  it('uses the default if flag not present in request query or cookies', () => {
    ...
    const res = mockRes()
    const next = jest.fn()
    readFeatureFlags({ testFlag: { default: true, description: 'TestDesc', label: 'Test' } })(req, res, next)
  ...
  it('overrides a default of false if flag is "1" in request query', () => {
    ...
    const res = mockRes()
    const next = jest.fn()
    readFeatureFlags({ testFlag: { default: false, description: 'TestDesc', label: 'Test' } })(req, res, next)
    expect(res.locals.flags).toEqual({
      testFlag: true,
    })
  })
