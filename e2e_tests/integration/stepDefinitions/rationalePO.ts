...
const openApp = (queryParams: object) => {
  let queryParameters = ''
  ...
  cy.visitPage(`?${queryParameters}`)
}
Before({ tags: '@Trigger' }, () => {
  openApp({ flagRecommendationsPage: 1, flagDeleteRecommendation: 1, flagTriggerWork: 1 })
})

function makeRecommendation(crn, recommendationDetails: Record<string, string>) {
  ...
  cy.get(`[data-qa="row-${crn}"] [data-qa="name"]`)
    ...
    .then(offenderName => {
      ...
      // get all licence conditions and choose a few randomly
...
Given('a PO has created a recommendation to recall', () => {
  const crn = crns[faker.helpers.arrayElement(Object.keys(crns))]
...
Given('requests an SPO to countersign', () => {
  ...
  cy.getElement('Sign out').then($signOut => {
    while ($signOut.is(':visible')) cy.clickLink('Sign out')
  })
})
